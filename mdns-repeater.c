...
struct if_sock {
    ...
    int v6_ok;                           /* joined ff02::fb successfully on this iface */
    struct in6_addr ll6;            /* cached IPv6 link-local address */
    int has_ll6;                    /* 1 if ll6 is valid */
};
...
/* ---- Lightweight duplicate suppression (200ms window, cross-interface & direction-aware) ---- */
#define DEDUPE_SLOTS 256
struct dedupe_entry { uint32_t h; uint64_t ts_ms; };
static struct dedupe_entry dedupe[DEDUPE_SLOTS];

static inline uint64_t now_ms(void){
    struct timespec t; clock_gettime(CLOCK_MONOTONIC, &t);
    return (uint64_t)t.tv_sec*1000ULL + (uint64_t)t.tv_nsec/1000000ULL;
}
static inline uint32_t djb_hash(const void *p, size_t n){
    const unsigned char* s=(const unsigned char*)p; uint32_t h=5381;
    for(size_t i=0;i<n;i++) h=((h<<5)+h) ^ s[i];
    return h;
}
/* Consider duplicates, but include recv_if and unicast/multicast in the hash salt so we don't drop a needed unicast after a multicast (or vice-versa). */
static int dedupe_hit(const void*buf,size_t len,int recv_if,int is_mcast){
    uint64_t t=now_ms();
    size_t clip = (len<64?len:64);
    uint32_t h=djb_hash(buf, clip);
    /* mix in lightweight salt: interface index and cast direction bit */
    uint32_t salt = (uint32_t)recv_if ^ (is_mcast?0xA5A5A5A5u:0x5A5A5A5Au);
    h ^= salt;
    int idx=(int)(h % DEDUPE_SLOTS);
    if (dedupe[idx].h==h && (t - dedupe[idx].ts_ms) < 200) return 1; /* duplicate */
    dedupe[idx].h=h; dedupe[idx].ts_ms=t; return 0;
}
...
static int create_recv_sock() {
    ...
    /* Larger buffers to reduce wakeups on bursts */
    int rcv = 512*1024, snd = 512*1024;
    setsockopt(sd, SOL_SOCKET, SO_RCVBUF, &rcv, sizeof(rcv));
    setsockopt(sd, SOL_SOCKET, SO_SNDBUF, &snd, sizeof(snd));

#ifdef IP_MULTICAST_ALL
    /* Only deliver multicast we explicitly joined */
    int off = 0;
    setsockopt(sd, IPPROTO_IP, IP_MULTICAST_ALL, &off, sizeof(off));
#endif
...
#ifdef IP_RECVTTL
    if ((r = setsockopt(sd, IPPROTO_IP, IP_RECVTTL, &on, sizeof(on))) < 0) {
        log_message(LOG_ERR, "recv setsockopt(IP_RECVTTL): %s", strerror(errno));
        return r;
    }
#endif
...
static int create_send_sock(int recv_sockfd, const char *ifname, struct if_sock *sockdata) {
    ...
    /* enable multicast loopback so local consumers (e.g., Avahi) see forwarded packets
     * Feedback is suppressed in recv path by dropping frames sourced from our own ifaddr. */
    int loop_on = 1;
    if ((r = setsockopt(sd, IPPROTO_IP, IP_MULTICAST_LOOP, &loop_on, sizeof(loop_on))) < 0) {
        log_message(LOG_ERR, "send setsockopt(IP_MULTICAST_LOOP on): %s", strerror(errno));
        return r;
    }
...
static ssize_t send_packet6_on(struct if_sock *ifs, const void *data, size_t len) {
    if (!ifs || ifs->sockfd6 <= 0 || !ifs->v6_ok) {
        errno = ENODEV; return -1;
    }
    int fd = ifs->sockfd6;
    /* Hint kernel which iface to use for multicast */
    int ifidx = ifs->ifindex;
    (void)setsockopt(fd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &ifidx, sizeof(ifidx));
    ...
    to6.sin6_scope_id = ifidx; /* link-local scope */

    ssize_t s = sendto(fd, data, len, 0, (struct sockaddr *)&to6, sizeof(to6));
    if (s < 0 && (errno == EADDRNOTAVAIL || errno == ENODEV)) {
        if (ifs->v6_ok) {
            ifs->v6_ok = 0; /* disable v6 on this iface until restart */
            log_message(LOG_INFO, "ipv6 disabled on %s (no link-local?)", ifs->ifname);
        }
    }
    return s;
}
...
static ssize_t send_packet6(int fd, int ifindex, const void *data, size_t len) {
    (void)fd; (void)ifindex; (void)data; (void)len; /* unused in new path */
    errno = ENOTSUP; return -1; /* legacy helper no longer used */
}

static ssize_t send_unicast6_to_on(struct if_sock *ifs, const struct sockaddr_in6 *to, const void *data, size_t len) {
    if (!ifs || ifs->sockfd6 <= 0 || !ifs->v6_ok) { errno = ENODEV; return -1; }
    struct sockaddr_in6 dst = *to;
    if (dst.sin6_port == 0) dst.sin6_port = htons(MDNS_PORT);
    if (IN6_IS_ADDR_LINKLOCAL(&dst.sin6_addr)) dst.sin6_scope_id = ifs->ifindex;
    /* Also hint outgoing IF for good measure */
    int ifidx = ifs->ifindex; (void)setsockopt(ifs->sockfd6, IPPROTO_IPV6, IPV6_MULTICAST_IF, &ifidx, sizeof(ifidx));
    ssize_t s = sendto(ifs->sockfd6, data, len, 0, (struct sockaddr *)&dst, sizeof(dst));
    if (s < 0 && (errno == EADDRNOTAVAIL || errno == ENODEV)) {
        if (ifs->v6_ok) { ifs->v6_ok = 0; log_message(LOG_INFO, "ipv6 disabled on %s (no link-local?)", ifs->ifname); }
    }
    return s;
}

static ssize_t send_unicast6_to(int fd, const struct sockaddr_in6 *to, const void *data, size_t len) {
    (void)fd; (void)to; (void)data; (void)len;
    errno = ENOTSUP; return -1;
}

/* Discover IPv6 link-local address for a given interface and cache it in struct if_sock */
static void populate_linklocal6(struct if_sock *ifs)
{
    if (!ifs || !ifs->ifname) return;
    ifs->has_ll6 = 0;
    struct ifaddrs *ifa = NULL;
    if (getifaddrs(&ifa) != 0) return;
    for (struct ifaddrs *p = ifa; p; p = p->ifa_next) {
        if (!p->ifa_name || !p->ifa_addr) continue;
        if (strcmp(p->ifa_name, ifs->ifname) != 0) continue;
        ...
        memcpy(&ifs->ll6, &sa6->sin6_addr, sizeof(struct in6_addr));
        ifs->has_ll6 = 1;
        break;
    }
    freeifaddrs(ifa);
...
static int create_recv_sock6() {
    ...
    /* Larger buffers */
    int rcv6 = 512*1024, snd6 = 512*1024;
    setsockopt(sd, SOL_SOCKET, SO_RCVBUF, &rcv6, sizeof(rcv6));
    setsockopt(sd, SOL_SOCKET, SO_SNDBUF, &snd6, sizeof(snd6));

#ifdef IPV6_MULTICAST_ALL
    int off6 = 0;
    setsockopt(sd, IPPROTO_IPV6, IPV6_MULTICAST_ALL, &off6, sizeof(off6));
#endif
    /* want pktinfo */
    ...
    if ((r = setsockopt(sd, IPPROTO_IPV6, IPV6_RECVHOPLIMIT, &on, sizeof(on))) < 0) {
        log_message(LOG_ERR, "recv6 setsockopt(IPV6_RECVHOPLIMIT): %s", strerror(errno));
        return r;
    }
    #endif
    return sd;
...
static int create_send_sock6(int recv_sockfd6, const char *ifname, struct if_sock *sockdata) {
    ...
    sockdata->sockfd6 = sd;
    sockdata->v6_ok = 0;
    sockdata->has_ll6 = 0; memset(&sockdata->ll6, 0, sizeof(sockdata->ll6));
    ...
    int rcv6 = 256*1024, snd6 = 512*1024;
    setsockopt(sd, SOL_SOCKET, SO_RCVBUF, &rcv6, sizeof(rcv6));
    setsockopt(sd, SOL_SOCKET, SO_SNDBUF, &snd6, sizeof(snd6));
    ...
    int loop6_on = 1;
    setsockopt(sd, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &loop6_on, sizeof(loop6_on));
    /* Preselect interface for multicast sends */
    if (sockdata->ifindex > 0) {
        int ifidx = sockdata->ifindex;
        setsockopt(sd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &ifidx, sizeof(ifidx));
    }
    ...
    /* Cache IPv6 link-local address for loop-prevention filtering */
    populate_linklocal6(sockdata);
...
int main(int argc, char *argv[]) {
    ...
    char cmsgbuf[CMSG_SPACE(sizeof(struct in_pktinfo)) + CMSG_SPACE(sizeof(int))];
    ...
    while (! shutdown_flag) {
        ...
        if (FD_ISSET(server_sockfd, &sockfd_set)) {
            ...
            int is_mcast = 1; /* assume mcast when kernel doesn't give us the group */
            if (dstaddr.sin_addr.s_addr != 0)
                is_mcast = IN_MULTICAST(ntohl(dstaddr.sin_addr.s_addr));
            ...
            /* Deduplicate: skip if duplicate recently seen */
            if (0 && dedupe_hit(pkt_data, (size_t)recvsize, recv_if, is_mcast)) continue;
 ...
 /* Accept non-255 TTL when it came via a tunnel (wg*). WG decrements inner TTL. */
int is_wg_in = 0;
for (int k = 0; k < num_socks; k++) {
    if (socks[k].ifindex == recv_if && socks[k].ifname && strncmp(socks[k].ifname, "wg", 2) == 0) {
        is_wg_in = 1; break;
    }
}
if (is_mcast && ttl != -1 && ttl != 255 && !is_wg_in) {
    if (log_verbosity > 0)
        log_message(LOG_DEBUG, "drop v4: TTL=%d mcast=%d ifindex=%d (non-wg path)", ttl, is_mcast, recv_if);
    continue;
}
            ...
            /* Record querier regardless of multicast detection so unicast replies can be steered */
            if (is_query && recv_sock_index >= 0) {
                for (int jrec = 0; jrec < num_socks; jrec++) {
                    if (jrec == recv_sock_index) continue; /* opposite iface(s) will use this */
                    socks[jrec].last_querier = fromaddr;
                    socks[jrec].last_querier_ts = time(NULL);
                    socks[jrec].last_querier_sock_index = recv_sock_index;
                }
            }
            ...
            if (is_mcast) {
                for (j = 0; j < num_socks; j++) {
                    ...
                    if (log_verbosity > 0)
    log_message(LOG_DEBUG, "send v4 mcast -> %s", socks[j].ifname);
...
ssize_t sentsize = send_packet(socks[j].sockfd, pkt_data, (size_t) recvsize);
        ...
        if (server_sockfd6 >= 0 && FD_ISSET(server_sockfd6, &sockfd_set)) {
            ...
            int hop = -1;
            char cmsgbuf6[CMSG_SPACE(sizeof(struct in6_pktinfo)) + CMSG_SPACE(sizeof(int))];
...
#ifdef IPV6_HOPLIMIT
                else if (cmsg6->cmsg_level == IPPROTO_IPV6 && cmsg6->cmsg_type == IPV6_HOPLIMIT) {
                    memcpy(&hop, CMSG_DATA(cmsg6), sizeof(hop));
                }
#endif
            }
            if (!pi6) goto after_ipv6;
            int recv_if = pi6->ipi6_ifindex;
            /* Drop frames that originate from any of our local IPv6 addresses on configured ifaces */
            for (int jdrop = 0; jdrop < num_socks; jdrop++) {
                if (socks[jdrop].has_ll6 && IN6_ARE_ADDR_EQUAL(&from6.sin6_addr, &socks[jdrop].ll6)) {
                    if (log_verbosity > 0) {
                        char selfbuf[INET6_ADDRSTRLEN];
                        inet_ntop(AF_INET6, &from6.sin6_addr, selfbuf, sizeof(selfbuf));
                        log_message(LOG_DEBUG, "drop v6 self-loop from %s on ifindex=%d", selfbuf, recv_if);
                    }
                    goto after_ipv6; /* ignore our own multicast/unicast */
                }
            }
            int is_mcast = IN6_IS_ADDR_MULTICAST(&pi6->ipi6_addr);
            /* Deduplicate: skip if duplicate recently seen */
            if (dedupe_hit(pkt_data, (size_t)n6, recv_if, is_mcast)) goto after_ipv6;
            /* Determine multicast vs unicast by dst */
            /* int is_mcast = IN6_IS_ADDR_MULTICAST(&pi6->ipi6_addr); -- removed duplicate */
            int is_wg6_in = 0;
for (int k = 0; k < num_socks; k++) {
    if (socks[k].ifindex == recv_if && socks[k].ifname && strncmp(socks[k].ifname, "wg", 2) == 0) {
        is_wg6_in = 1; break;
...
if (is_mcast && hop != -1 && hop != 255 && !is_wg6_in) goto after_ipv6;
            int j;
            if (log_verbosity > 0) {
            ...
            if (is_mcast) {
                for (j = 0; j < num_socks; j++) {
                    ...
                    if (!socks[j].v6_ok) continue;      /* skip if iface has no IPv6 mcast */
                    if (log_verbosity > 0) log_message(LOG_DEBUG, "repeating v6 mcast to %s", socks[j].ifname);
                    ssize_t s = send_packet6_on(&socks[j], pkt_data, (size_t)n6);
                    if (s != n6) {
                        if (s < 0) log_message(LOG_ERR, "send6: %s", strerror(errno));
                        else log_message(LOG_ERR, "send_packet6 size differs: sxent=%zd actual=%zd", n6, s);
                    }
                    if (is_query) {
            ...
            if (is_response && recv_sock_index >= 0) {
                ...
                if (in_if->last_querier6.sin6_family == AF_INET6 && in_if->last_querier6_ts && (now - in_if->last_querier6_ts) <= QUERIER_TTL) {
                    ...
                    if (out_idx >= 0 && out_idx < num_socks && socks[out_idx].sockfd6 > 0) {
                        ssize_t s = send_unicast6_to_on(&socks[out_idx], &in_if->last_querier6, pkt_data, (size_t)n6);
                        if (s != n6) {
